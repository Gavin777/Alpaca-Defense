...
public class World {
	...
	final List<Powwwwup> powerup= new ArrayList<Powwwwup>();
	final Ship ship;
	long lastShotTime;
	Random random;
	...
	private void generateInvaders() {
		...
		Powwwwup powwwwup = new Powwwwup(0,
				0, -5);
		powerup.add(powwwwup);
	}
	...
	private void checkShotCollisions() {
		...
		for (int i = 0; i < len; i++) {
			...
			for (int z=0; z<len2;z++) {
				Powwwwup powwwwup = powerup.get(z);
				if (OverlapTester.overlapSpheres(powwwwup.bounds,  shot.bounds) && powwwwup.state == Powwwwup.INVADER_ALIVE ){
					//powwwwup.kill();
					powwwwup.state = powwwwup.INVADER_DEAD; 
					powerup.remove(z);
					listener.explosion();
					score += 100;
					ship.lives++;
	...
	public void shot() {
		...
		if (score > waves*400){ 
			if (System.nanoTime() - lastShotTime > 1000000 || friendlyShots == 0) {
