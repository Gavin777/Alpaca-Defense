...
//		ambientLight.enable(gl);
		...
		renderPowerup(gl, world.powerup, deltaTime);
	...
	private void renderPowerup(GL10 gl, List<Powwwwup> Powerup, float deltaTime) {
		buffAngle += 90 * deltaTime;

		Assets.invaderTexture.bind();
		Assets.invaderModel.bind();
		int len = Powerup.size();
		for (int i = 0; i < len; i++) {
			Powwwwup powwwwup = Powerup.get(i);
			
			if (powwwwup.state == Powwwwup.INVADER_DEAD) {
				gl.glDisable(GL10.GL_LIGHTING);
				Assets.invaderModel.unbind();
				renderExplosion(gl, powwwwup.position, powwwwup.stateTime);
				Assets.invaderTexture.bind();
				Assets.invaderModel.bind();
				gl.glEnable(GL10.GL_LIGHTING);
			} else {
				gl.glPushMatrix();
				gl.glTranslatef(powwwwup.position.x, powwwwup.position.y,
						powwwwup.position.z);
				gl.glRotatef(buffAngle, 0, 1, 0);
				Assets.invaderModel.draw(GL10.GL_TRIANGLES, 0,
						Assets.invaderModel.getNumVertices());
				gl.glPopMatrix();
			}
